...
export class Engine extends THREE.Group {
  constructor(renderer) {
    super();
    {  
      const hemi = new THREE.HemisphereLight(0xe8f3ff, 0x5f6870, 0.5);
      this.add(hemi);
      ...
      const fill = new THREE.DirectionalLight(0xe7eeff, 0.9);
      fill.position.set(-12, 8, -10);
      this.add(fill);

      const rim = new THREE.DirectionalLight(0xc8ddff, 0.55);
    ...
    this._onKill = (ball) => {
      if (ball === this.currentClosestBall) {
        // Switch to new closest ball after kill
  ...
  setMode(newMode) {
    ...
    this._onHit = () => this.currentMode.onHit();
    this._onKill = () => this.currentMode.onKill();
    this.BallManager.on('hit', this._onHit);
    this.BallManager.on('killed', this._onKill);
  }
